...
usage() {
  cat <<USAGE
Usage:
  bash scripts/learning/run_learning_domestic_transmission_prereqs_job.sh TD0|TD1 [--dry-run]

Rebuilds only the transmission-sensitive outputs for a domestic-grid sensitivity:
...
case "$CASE" in
  TD0|TD1)
    ...
    ;;
  *)
    echo "Unsupported domestic transmission case: $CASE (expected TD0 or TD1)" >&2
    exit 1
    ;;
esac
...
RUNTIME_OVERLAY="$(mktemp "$TMPDIR_ROOT/energymod_learning_domestic_tx.XXXXXX.yaml")"
...
echo "Running domestic transmission prerequisites: case=$CASE sector_name=$SECTOR_NAME dry_run=$DRY_RUN"
cd "$ROOT_DIR"
